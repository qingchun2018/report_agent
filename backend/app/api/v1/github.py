...
async def get_github_trending(db: DatabaseDep, _user: CurrentUserDep, days: int = Query(7)):
    stats = StatsService(db)
    return {"data": await stats.get_github_trending(days)}


@router.get("/github/star-history/{repo_name}")
async def get_star_history(db: DatabaseDep, _user: CurrentUserDep, repo_name: str, days: int = Query(30)):
    stats = StatsService(db)
    return {"data": await stats.get_github_star_history(repo_name, days)}


@router.get("/github/anomalies")
async def get_anomalies(db: DatabaseDep, _user: CurrentUserDep):
    stats = StatsService(db)
