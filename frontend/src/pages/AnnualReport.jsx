...
export default function AnnualReport() {
  ...
  const loadKpi = async () => {
    try {
      const data = await apiJson('/api/annual/github/kpi', { silent: true });
      setKpi(data.years || {});
    } catch { /* ignore */ }
  };
  ...
  return (
    <div className="max-w-7xl space-y-6">
      ...
      {/* GitHub 暗色 GitHub KPI 总览卡 */}
      {kpi && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {Object.entries(kpi).sort(([a],[b]) => Number(a)-Number(b)).map(([year, d]) => {
            const curr = d.total || 0;
            const prevYear = String(Number(year) - 1);
            const prev = kpi[prevYear]?.total || 0;
            const diff = curr - prev;
            const pct = prev > 0 ? ((diff / prev) * 100).toFixed(1) : '—';
            const isUp = diff >= 0;
            return (
              <div key={year}
                className="bg-[#161b22] border border-[#30363d] rounded-2xl p-4 shadow-sm hover:border-[#238636]/30 transition-colors"
              >
                <div className="flex items-center gap-1.5 mb-2">
                  <svg className="w-3.5 h-3.5 fill-[#8b949e]" viewBox="0 0 16 16"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z"/></svg>
                  <span className="text-[11px] font-medium text-[#8b949e] uppercase tracking-wide">{year}</span>
                </div>
                <p className="text-2xl font-extrabold text-[#f0f6fc] tabular-nums tracking-tight">{fmtKpi(curr)}</p>
                <p className="text-[10px] text-[#484f58] mt-1">{d.repos || 0} repos</p>
                {year !== '2021' && (
                  <div className="mt-2 pt-2 border-t border-[#21262d]">
                    <div className="flex items-center gap-1.5">
                      <span className={`text-xs font-bold ${isUp ? 'text-[#3fb950]' : 'text-[#f85149]'}`}>
                        {isUp ? '↑' : '↓'} {isUp ? '+' : ''}{fmtKpi(Math.abs(diff))}
                      </span>
                      <span className={`text-[10px] ${isUp ? 'text-[#3fb950]/70' : 'text-[#f85149]/70'}`}>
                        ({isUp ? '+' : ''}{pct}%)
                      </span>
                    </div>
                    <p className="text-[9px] text-[#484f58]">vs {prevYear}: {fmtKpi(prev)}</p>
                  </div>
                )}
              </div>
            );
