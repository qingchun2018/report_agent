...
- **账号体系**：注册 / 登录 / 修改密码 / 个人中心；JWT 鉴权；**首位注册用户自动成为管理员**
- **管理员后台**：用户列表、启用/禁用、删除（管理员侧边栏可见）
- **Dashboard**：漏洞总览、趋势、GitHub 热门、异常检测
- **OpenRank / Hive / 年报**：指标与表差异、年度对比
- **Reports**：日报 / 周报 / 月报；支持「打印 / 导出 PDF」
- **AI Agent**：自然语言查库、图表建议、执行轨迹；支持 `session_id` 多轮与复合问题分解；会话按用户**本地持久化**
- **全局体验**：Toast 错误提示、骨架屏 Loading、暗色模式、返回顶部、404 页面
- **安全增强**：登录失败次数限制（5 次锁 15 分钟）、注册 IP 限频、**改密后旧 token 立即失效**
...
- 账号鉴权所需环境变量：`JWT_SECRET_KEY`（生产务必改为长随机串）、`JWT_EXPIRE_MINUTES`（默认 7 天）

> 首次启动后访问任意页面会被引导到 `/login`，点「立即注册」创建第一个账号，**第一个注册的账号自动成为管理员**，可在侧边栏「用户管理」中管理后续注册的用户。
